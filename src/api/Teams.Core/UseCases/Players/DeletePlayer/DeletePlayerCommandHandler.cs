...
public class DeletePlayerCommandHandler(
    ...
    ILogger<DeletePlayerCommandHandler> logger)
    : IRequestHandler<DeletePlayerCommand, Player>
{
    public async Task<Player> HandleAsync(DeletePlayerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (player.UserId is null) actor.Current.ThrowIfNotOrganiser(player.Game.OrganiserId);
        else actor.Current.ThrowIfNotOrganiserOrUser(player.UserId, player.Game.OrganiserId);
