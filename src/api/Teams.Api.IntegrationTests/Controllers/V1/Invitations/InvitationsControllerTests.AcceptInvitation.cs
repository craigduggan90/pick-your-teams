...
{
    public class AcceptInvitation(ApiWebApplicationFactory factory) : InvitationsControllerTestsBase(factory)
    {
        [Fact]
        public async Task ShouldReturnBadRequest_WhenVersionIsUnsupported()
        {
            ...
            var request = CreateRequest(HttpMethod.Post, $"{VersionlessUrl}/{existingInvitation.Id}", apiVersion: "2.0");
        ...
        public async Task ShouldReturnBadRequest_WhenVersionIsNotProvided()
        {
            ...
            var request = CreateRequest(HttpMethod.Post, $"{VersionlessUrl}/{existingInvitation.Id}", apiVersion: null);
            WithActorHeaders(request, invitee);
        ...
        public async Task ShouldReturnPreconditionRequired_WhenActorHeadersAreMissing()
        {
            ...
            var request = CreateRequest(HttpMethod.Post, $"{Url}/{existingInvitation.Id}");
        ...
        public async Task ShouldReturnNotFound_WhenInvitationDoesNotExist()
        {
            ...
            var request = CreateRequest(HttpMethod.Post, $"{Url}/{id}");
        ...
        [Fact]
        public async Task ShouldReturnUnprocessableEntity_WhenInvitationIsAlreadyDeclined()
        {
        ...
        [Fact]
        public async Task ShouldReturnNoContent_WithUnchangedStatus_WhenAlreadyAccepted()
        {
            ...
            Assert.Equal(nameof(InvitationStatusEnum.Accepted), detail.Status);
        }
        ...
        [Fact]
        public async Task ShouldReturnNoContent_WithPlayerCreated_WhenRequestIsValid()
        {
            var (game, invitee, invitation) = await SeedOpenInvitationAsync();
            ...
            var getInvitationRequest = CreateRequest(HttpMethod.Get, $"{Url}/{invitation.Id}");
            WithActorHeaders(getInvitationRequest, invitee);
            var getInvitationResponse = await Client.SendAsync(getInvitationRequest, TestContext.Current.CancellationToken);
            var detail = await ReadContentAsync<InvitationDetailModel>(getInvitationResponse, TestContext.Current.CancellationToken);

            Assert.NotNull(detail);
            Assert.Equal(nameof(InvitationStatusEnum.Accepted), detail.Status);

            var getPlayersUrl = WithQuery("api/v1/players", ("GameId", game.Id), ("UserId", invitee.Id));
            var getPlayersRequest = CreateRequest(HttpMethod.Get, getPlayersUrl);
            var getPlayersResponse = await Client.SendAsync(getPlayersRequest, TestContext.Current.CancellationToken);
            var players = await ReadContentAsync<PagedList<PlayerModel>>(getPlayersResponse, TestContext.Current.CancellationToken);
